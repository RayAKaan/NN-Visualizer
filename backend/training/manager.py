    ...
    def _run_training(self) -> None:
        ...
        for epoch in range(1, self._config.epochs + 1):
            ...
            for batch_index, (batch_x, batch_y) in enumerate(dataset):
                ...
                acc = float(accuracy.numpy())

                epoch_losses.append(loss)
                epoch_accs.append(acc)

                activations_raw = activation_model.predict(batch_x[:1], verbose=0)
                weights_raw = model.get_weights()
                grad_h1_h2 = grads[2].numpy() if grads[2] is not None else np.zeros_like(weights_raw[2])
                grad_h2_out = grads[4].numpy() if grads[4] is not None else np.zeros_like(weights_raw[4])

                activations = {
                    "hidden1": activations_raw[0][0].tolist(),
                    "hidden2": activations_raw[1][0].tolist(),
                    "output": activations_raw[2][0].tolist(),
                }
                gradients = {
                ...
                weights = {
                    "hidden1_hidden2": weights_raw[2].tolist(),
                    "hidden2_output": weights_raw[4].tolist(),
                }
