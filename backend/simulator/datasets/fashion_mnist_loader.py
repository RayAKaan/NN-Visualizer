...
def load_fashion_mnist(n_samples: int, train_split: float, seed: int | None = None) -> Dict:
    if seed is not None:
        np.random.seed(seed)
    ...
    try:
        from tensorflow.keras.datasets import fashion_mnist as keras_fashion_mnist
        (x_train, y_train), (x_test, y_test) = keras_fashion_mnist.load_data()
    except Exception:
        data_path = os.path.join("models_store", "fashion_mnist.npz")
        if not os.path.exists(data_path):
            raise FileNotFoundError("Fashion-MNIST data not found. Place fashion_mnist.npz in models_store or ensure tensorflow is available.")
