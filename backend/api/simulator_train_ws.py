...
async def simulator_train_ws(ws: WebSocket):
    ...
    async def _safe_send(message: dict):
        # Never let a dead/closing socket crash the training task.
        try:
    ...
    try:
        while True:
            ...
            if action == "start":
                # Cancel any in-flight training first so runs can never pile up
                # and interleave their epoch/batch updates.
                if training_task is not None and not training_task.done():
                    stop_flag = True
                    try:
                        stale_session = training_sessions.get(current_graph_id) if current_graph_id else None
                        if stale_session is not None:
                            stale_session.is_paused = False
                    except KeyError:
                        pass
